...
def make_env(task_name='reach-v3', rank=0, seed=0, max_episode_steps=750, normalize_reward=False, terminate_on_success=False):
    """
...
class MultiTaskOneHotWrapper(gym.Env):
    ...
    """
    def __init__(self, envs, task_probs=None, seed=None):
        super().__init__()
        ...
        self.envs = [
            SingleTaskOneHotWrapper(env, task_id=i, n_tasks=self.n_tasks) for i, env in enumerate(envs)
        ]
...
def make_multitask_env(task_list, rank=0, seed=0, max_episode_steps=750, normalize_reward=False, task_probs=None):
    """
    ...
    def _init():
        ...
        env = MultiTaskOneHotWrapper(envs, task_probs=task_probs, seed=seed)
        return env
...
if __name__ == "__main__":
    ...
    MAX_EPISODE_STEPS = 750  # Maximum steps per episode
    ...
    env = SubprocVecEnv(
        [make_multitask_env(TASK_LIST, rank=i, seed=SEED, max_episode_steps=MAX_EPISODE_STEPS, normalize_reward=NORMALIZE_REWARD) for i in range(N_ENVS)],
        start_method='spawn'
    )
